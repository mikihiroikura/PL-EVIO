...
int main(int argc, char **argv)
{
    ...
    // if(FISHEYE)
    // {
    //     //for (int i = 0; i < NUM_OF_CAM_stereo; i++)
    //     {
...
void sync_process()
{
    while(1)
    {
        if(STEREO)
        {
            ...
            if(!image1.empty())
                handle_stereo_image(image1, image2, msg_timestamp); //进行双目的处理
        }
...
cv::Mat getImageFromMsg(const sensor_msgs::ImageConstPtr &img_msg)//额外对图片的尺寸进行检验
{
    ...
    if(!(ret_img.rows==ROW && ret_img.cols==COL)){//对付image有时大小不一致的问题

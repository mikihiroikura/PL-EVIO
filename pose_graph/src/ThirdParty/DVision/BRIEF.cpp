    ...
    bool treat_image) const
{
  ...
  const cv::Size ksize(9, 9);//高斯滤波用的
  ...
  if(treat_image)
  {
    ...
    if(image.depth() == 3)
    {
      cv::cvtColor(image, aux, cv::COLOR_RGB2GRAY);//把图像变为灰度（原本是CV_RGB2GRAY）
    }
    else//若不是3通道的图像则不需要变换
    {
      aux = image;
    }

    cv::GaussianBlur(aux, im, ksize, sigma, sigma);//高斯滤波
    
  }
  else//treat_image没有输入，所以应该是默认为0，直接原图=im
  {
    im = image;
  }
  
  assert(im.type() == CV_8UC1);
  assert(im.isContinuous());//是否连续存储？

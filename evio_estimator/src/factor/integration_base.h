...
class IntegrationBase
{
  public:
    ...
    /**
     * @brief 相关时间差和传感器数据保留，方便后续repropagate
     * 
     */
...
//做中值积分的时候，不仅仅更新了状态量，也更新了协方差矩阵及雅可比
// 雅可比矩阵就是当前构建的残差，对优化变量的导数
// 然后延着负方向与迭代优化,最终这个残差也可以达到最优了
// 而此处的雅可比矩阵是预积分值对各状态误差的导数
// 因为优化的时候,需要用到当前预积分值的协方差,而要推倒预积分的协方差,我们需要知道imu噪声和预积分量之间的线性递推
// 其中,IMU的测量误差可以通过标定的参数得到,而预积分是一段时间内的imu数据积累,其误差误差需要通过误差递推得到
// 而递推过程中,误差主要有两个部分产生:1,当前时刻的误差传递给下一时刻;2,当前时刻的测量噪声传递给下一时刻
// 而由于bias是优化变量,如果每次优化后,需要重新计算误差,那就带来很大的计算量
// 但通过得到雅可比矩阵,就可以建立预积分值根bias之间的关系,从而当它变化时,可以直接用,获得更新噪声后的预积分值
// 此处的雅可比矩阵，是预积分的值对状态的导数，其实就是为了，当状态变化时，计算出预积分的值变化多少
        if(update_jacobian)
        {

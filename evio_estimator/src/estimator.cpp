...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    if (frame_count != 0)//滑动窗口的第一帧是起点帧，没有预积分值
    {
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const std_msgs::Header &header)
{
    ...
    if (f_manager.addFeatureCheckParallax(frame_count, image, td))//关键帧的选择（添加特征并且检查视差，看是否作为keyframe）
    ...
    imageframe.pre_integration = tmp_pre_integration;//这段时间IMU预积分的信息
    all_image_frame.insert(make_pair(header.stamp.toSec(), imageframe));
    tmp_pre_integration = new IntegrationBase{acc_0, gyr_0, Bas[frame_count], Bgs[frame_count]};

    if(ESTIMATE_EXTRINSIC == 2)//自主估计IMU与camera的外参
    {
    ...
    if (solver_flag == INITIAL)//初始化过程
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            {
               result = initialStructure();//视觉与IMU对齐，求取bias，重力，速度，尺度
               initial_timestamp = header.stamp.toSec();
            }
            if(result)
            {
                solver_flag = NON_LINEAR;
                solveOdometry();//三角化求深度+非线性优化求解VIO
                slideWindow();//滑动窗口
                f_manager.removeFailures();// 移除无效地图点  （没有fusion中的去除outliers，是否可以加上看看效果？）
                ROS_INFO("Initialization finish!");
                last_R = Rs[WINDOW_SIZE];//获取滑动窗口内最新的位置
    ...
    else//非线性优化过程
    {
        ...
        TicToc t_margin;
        slideWindow();
        f_manager.removeFailures();//去除深度检测失败的点
...
//多了个线状特征
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, //点状特征
                             ...
                             const std_msgs::Header &header)
{
    ...
    ROS_DEBUG("Adding feature points %lu", image.size());
    // if (f_manager.addFeatureCheckParallax(frame_count, image, td))//关键帧的选择（添加特征并且检查视差，看是否作为keyframe）
    if (f_manager.addFeatureCheckParallax(frame_count, image, lines, td))//添加特征点与线特征, 检测视差 (根据视差决定marge最老帧还是次新帧)
        marginalization_flag = MARGIN_OLD;
    else
        marginalization_flag = MARGIN_SECOND_NEW;
    ...
    if (solver_flag == INITIAL)//初始化过程
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            if(result)
            {
                solver_flag = NON_LINEAR;
                // solveOdometry();//三角化求深度+非线性优化求解VIO
                solveOdometry_withline();  //(加入了线特征了) 
                slideWindow();//滑动窗口
                f_manager.removeFailures();// 移除无效地图点  （没有fusion中的去除outliers，是否可以加上看看效果？）
                ROS_INFO("Initialization finish!");
                last_R = Rs[WINDOW_SIZE];//获取滑动窗口内最新的位置
...
//image+event+event line
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, //图片特征
                             ...
                             const std_msgs::Header &header)
{
    ...
    // if (f_manager.addFeatureCheckParallax(frame_count, image, td))//关键帧的选择（添加特征并且检查视差，看是否作为keyframe）
    // if (f_manager.addFeatureCheckParallax(frame_count, image, lines, td))//添加特征点与线特征, 检测视差 (根据视差决定marge最老帧还是次新帧)
    if (f_manager.addFeatureCheckParallax(frame_count, image, event, lines, td))//添加特征点与线特征, 检测视差 (根据视差决定marge最老帧还是次新帧)
        marginalization_flag = MARGIN_OLD;
    else
        marginalization_flag = MARGIN_SECOND_NEW;
    ...
    if (solver_flag == INITIAL)//初始化过程
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            if(result)
            {
                ...
                // solveOdometry();//三角化求深度+非线性优化求解VIO
                // solveOdometry_withline();  //(加入了线特征了) 
                solveOdometry_plevio();  //(图像特征+事件点线特征) 
                slideWindow();//滑动窗口
                f_manager.removeFailures();// 移除无效地图点  （没有fusion中的去除outliers，是否可以加上看看效果？）
                ROS_INFO("Initialization finish!");
                last_R = Rs[WINDOW_SIZE];//获取滑动窗口内最新的位置
...
void Estimator::Setreo_processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, //图片特征
                             const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &event, //事件点状特征
                             const map<int, vector<pair<int, Vector4d>>> &lines,//事件线状特征
                             const std_msgs::Header &header)
{
    ROS_DEBUG("new image coming ------------------------------------------");
    ROS_DEBUG("Adding feature points %lu", image.size());
    if (f_manager.stereo_addFeatureCheckParallax(frame_count, image, event, lines, td))//添加特征点与线特征, 检测视差 (根据视差决定marge最老帧还是次新帧)
        marginalization_flag = MARGIN_OLD;
    else
        marginalization_flag = MARGIN_SECOND_NEW;
    ...
    if (solver_flag == INITIAL)//初始化过程
    {
        if (frame_count == WINDOW_SIZE)
        {
            ...
            if(result)
            {
                ...
                solveOdometry_stereo_plevio();  //(图像特征+事件点线特征) 
                slideWindow();//滑动窗口
                f_manager.removeFailures();// 移除无效地图点  （没有fusion中的去除outliers，是否可以加上看看效果？）
                ROS_INFO("Initialization finish!");
                last_R = Rs[WINDOW_SIZE];//获取滑动窗口内最新的位置
...
// bool Estimator::initialStructure_QPEP()
// {
//     TicToc t_sfm;
...
//            std::isnan(t.at<double>(0, 0)) ||
//            std::isnan(t.at<double>(1, 0)) ||
//            std::isnan(t.at<double>(2, 0)))
//         {
//             return false;
//         }
...
bool Estimator::initialStructure()
{
    ...
    map<int, Vector3d> sfm_tracked_points;
    vector<SFMFeature> sfm_f;
    for (auto &it_per_id : f_manager.feature)//此处单单采用了图像角点或事件角点。
    {
        int imu_j = it_per_id.start_frame - 1;
        SFMFeature tmp_feature;
        tmp_feature.state = false;
...
bool Estimator::initialStructureStereo()
{
    TicToc t_sfm;
    //check imu observibility
    {
    ...
    map<int, Vector3d> sfm_tracked_points;
    vector<SFMFeature> sfm_f;
    for (auto &it_per_id : f_manager.stereo_feature)
    {
        int imu_j = it_per_id.start_frame - 1;
        SFMFeature tmp_feature;
        tmp_feature.state = false;
...
bool Estimator::visualInitialAlignWithDepth()
{
    ...
    VectorXd dep = f_manager.stereo_getDepthVector();
    for (int i = 0; i < dep.size(); i++)
        dep[i] = -1;
    f_manager.clearDepth(dep);
    ...
    ric[0] = RIC[0];
    f_manager.setRic(ric);
    f_manager.stereo_triangulate(Ps, &(TIC_TMP[0]), &(RIC[0]));
    ...
    if(cnt > 0)
        ROS_INFO("total depth rmse: %lf ", sqrt(totoal_dpt_err/cnt));
...
bool Estimator::relativePoseHybrid(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    // find previous frame which contians enough correspondance and parallex with newest frame
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
...
{
    if (frame_count < WINDOW_SIZE)// 保证滑窗中帧数满了
        return;
    if (solver_flag == NON_LINEAR)// 其次要求初始化完成
    {
        TicToc t_tri;
        //利用初始位姿，对每帧图像还有未具有深度信息的特征点进行三角化
        // 后续准备利用imu传播的最新位姿构建重投影误差，对位姿进行优化
        f_manager.triangulate(Ps, tic, ric); // 先把应该三角化但是没有三角化的特征点三角化
        ROS_DEBUG("triangulation costs %f", t_tri.toc());
        optimization();//根据最新的视觉观测及imu信息在滑动窗口内对位姿进行优化
    }
}

// 点线特征一起处理
void Estimator::solveOdometry_withline()
{
    if (frame_count < WINDOW_SIZE)//要塞满windows
        return;

    if (solver_flag == NON_LINEAR)//开始进行非线性优化了
    {
        f_manager.triangulate(Ps, tic, ric);//对点特征进行三角化处理
        f_manager.triangulateLine(Ps, tic, ric);//对线特征进行三角化处理
        // optimization();
        // onlyLineOpt();   // 三角化以后，优化一把 (为什么好像优化了两次？)
        optimizationwithLine();
    }
}

//solveOdometry_plevio：图像特征+事件点线特征
void Estimator::solveOdometry_plevio()
{
    if (frame_count < WINDOW_SIZE)//要塞满windows
        return;
    ...
    if (solver_flag == NON_LINEAR)//开始进行非线性优化了
    {
        f_manager.triangulate(Ps, tic, ric);//对图像点特征进行三角化处理
        f_manager.Event_triangulate(Ps, tic, ric);//对事件点特征进行三角化处理
        f_manager.triangulateLine(Ps, tic, ric);//对事件线特征进行三角化处理
        // optimization();
        // onlyLineOpt();   // 三角化以后，优化一把 (为什么好像优化了两次？)
        // optimizationwithLine();
        optimization_plevio();
...
// solveOdometry_stereo_plevio
void Estimator::solveOdometry_stereo_plevio()
{
    if (frame_count < WINDOW_SIZE)//要塞满windows
        return;
    ...
    {
        // f_manager.triangulate(Ps, tic, ric);//对图像点特征进行三角化处理
        f_manager.triangulateStereo();//先获取双目的深度
        f_manager.stereo_triangulate(Ps, tic, ric);//再单目计算一下
...
//增加了事件线特征以及当有图像的时候，把feature增加了事件特征
void Estimator::vector2double()
{
    ...
    // 事件点特征
    VectorXd dep_event = f_manager.Event_getDepthVector();
    for (int i = 0; i < f_manager.getEventFeatureCount(); i++)
        para_EventFeature[i][0] = dep_event(i);
...
void Estimator::stereo_vector2double()
{
    for (int i = 0; i <= WINDOW_SIZE; i++)
    {
        para_Pose[i][0] = Ps[i].x();
    ...
    //图像特征点
    VectorXd dep = f_manager.stereo_getDepthVector();
    for (int i = 0; i < f_manager.stereo_getFeatureCount(); i++)
        para_Feature[i][0] = dep(i);
    //估算时延
    if (ESTIMATE_TD)
        para_Td[0][0] = td;
...
void Estimator::double2vector()
{
    ...
    { 
        ...
        drift_correct_yaw = Utility::R2ypr(prev_relo_r).x() - Utility::R2ypr(relo_r).x();//yaw角度的累积误差
        drift_correct_r = Utility::ypr2R(Vector3d(drift_correct_yaw, 0, 0));//得到旋转的累积误差
        drift_correct_t = prev_relo_t - drift_correct_r * relo_t; //得到平移的累积误差  
...
void Estimator::double2vector2()
{
    ...
    // 优化后的第一帧的位姿 (优化以后的0th的姿态)
    Vector3d origin_R00 = Utility::R2ypr(Quaterniond(para_Pose[0][6],
                                                      para_Pose[0][3],
                                                      para_Pose[0][4],
                                                      para_Pose[0][5]).toRotationMatrix());
    // yaw角差
    double y_diff = origin_R0.x() - origin_R00.x();//优化前后，yaw的变化
    //TODO
    Matrix3d rot_diff = Utility::ypr2R(Vector3d(y_diff, 0, 0));
    ...
    for (int i = 0; i <= WINDOW_SIZE; i++)
    {
        // 保持第1帧的yaw不变
        Rs[i] = rot_diff * Quaterniond(para_Pose[i][6], para_Pose[i][3], para_Pose[i][4], para_Pose[i][5]).normalized().toRotationMatrix();
        // 保持第1帧的位移不变  Position 也转移到yaw角优化前的 0th坐在的世界坐标下
        Ps[i] = rot_diff * Vector3d(para_Pose[i][0] - para_Pose[0][0],
                                para_Pose[i][1] - para_Pose[0][1],
                                para_Pose[i][2] - para_Pose[0][2]) + origin_P0;
    ...
    // 原本还有 // relative info between two loop frame
    if(relocalization_info)
    { 
        Matrix3d relo_r;
        Vector3d relo_t;
...
void Estimator::double2vector3()
{
    ...
    VectorXd dep_event = f_manager.Event_getDepthVector();
    for (int i = 0; i < f_manager.getEventFeatureCount(); i++)
        dep_event(i) = para_EventFeature[i][0];
    f_manager.Event_setDepth(dep_event);
    ...
    { 
        ...
        //此时将前端的姿态变换一下  
        // for (int i = 0; i <= WINDOW_SIZE; i++){
        //     Ps[i] = drift_correct_r * Ps[i] + drift_correct_t;
        //     Rs[i] = drift_correct_r * Rs[i];
        // }
    }
    
}

void Estimator::stereo_double2vector3()
{
    // 六自由度优化的时候，整个窗口会在空间中任意优化，这时候我们需要把第一帧在yaw,position上的增量给去掉，因为vins在这几个方向上不可观，他们优化的增量也不可信。
    // 所以这里的操作过程就相当于是 fix 第一帧的 yaw 和 postion, 使得整个轨迹不会在空间中任意飘。
    // 相机姿态需要变化考虑优化以后，把yaw量旋转回去
...
bool Estimator::failureDetection()
{
   ...
   //下面是看看运动是否太aggressive的，本身EVIO就应该aggressive一些，所以不要
...
{   
   ...
   // 对于线特征
    ...
    for (auto &it_per_id : f_manager.linefeature)//对于线特征
    {
        ...
        int imu_i = it_per_id.start_frame, imu_j = imu_i - 1;//frame index// // 第一个观测到这个特征点的帧idx

        // for (auto &it_per_frame : it_per_id.feature_per_frame)//遍历所有的特征
        for (auto &it_per_frame : it_per_id.linefeature_per_frame)//遍历所有线特征
        {
            imu_j++;
            ...
            Vector4d obs = it_per_frame.lineobs;   // 在第j帧图像上的观测 （// 取出另一帧的归一化相机坐标）
    ...
    if (line_m_cnt!=0)
        ROS_WARN("lineFactor: %d, pointFactor:%d in the optimization", line_m_cnt, f_m_cnt);
    ...
    if (marginalization_flag == MARGIN_OLD)
    {
        ...
        {  // Line feature
            int linefeature_index = -1;
            for (auto &it_per_id : f_manager.linefeature)// 遍历features
            {
                it_per_id.used_num = it_per_id.linefeature_per_frame.size();                // 已经被多少帧观测到， 这个已经在三角化那个函数里说了
...
//(optimization_plevio) 图像特征+事件点线特征进行优化
void Estimator::optimization_plevio()
{   
    frame_cnt_ ++;
    ceres::Problem problem;
    ceres::LossFunction *loss_function;
    ...
    for (auto &it_per_id : f_manager.Event_feature)
    {
        ...
        Vector3d pts_i = it_per_id.Event_feature_per_frame[0].point;
        ...
        for (auto &it_per_frame : it_per_id.Event_feature_per_frame)
        {
            ...
            if (ESTIMATE_TD)
            {
                ProjectionTdFactor *f_td = new ProjectionTdFactor(pts_i, pts_j, it_per_id.Event_feature_per_frame[0].velocity, it_per_frame.velocity,
                                                                    it_per_id.Event_feature_per_frame[0].cur_td, it_per_frame.cur_td,
                                                                    it_per_id.Event_feature_per_frame[0].uv.y(), it_per_frame.uv.y());
                problem.AddResidualBlock(f_td, loss_function, para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[0], para_EventFeature[Event_feature_index], para_Td[0]);
            }
            else
            {
                ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j);
                problem.AddResidualBlock(f, loss_function, para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[0], para_EventFeature[Event_feature_index]);
            }
            Event_f_m_cnt++;
        }
    }
   ...
   // 对于事件线特征
    ...
    // if (f_m_cnt!=0)
        // ROS_WARN("event pointFactor: %d, event lineFactor: %d, image pointFactor:%d in the optimization", Event_f_m_cnt, line_m_cnt, f_m_cnt);
    ...
    {
        // printf("set relocalization factor! \n");
        // ROS_ERROR("add the loop constraint !!!!!!");
        ceres::LocalParameterization *local_parameterization = new PoseLocalParameterization();
        problem.AddParameterBlock(relo_Pose, SIZE_POSE, local_parameterization);
        int retrive_feature_index = 0;
        int feature_index = -1;
    ...
    options.linear_solver_type = ceres::DENSE_SCHUR;
    // options.num_threads = 3; //2;//启用多线程（毕竟三种特征一起，数量太多了）
    options.trust_region_strategy_type = ceres::DOGLEG;
    options.max_num_iterations = NUM_ITERATIONS;
    //options.use_explicit_schur_complement = true;
    //options.minimizer_progress_to_stdout = true;
    ...
    // double2vector();
    // double2vector2();   // Line pose change
    double2vector3();   // Line pose change
    // f_manager.removeLineOutlier(Ps,tic,ric);   // remove Line outlier
                ...
                for (auto &it_per_frame : it_per_id.Event_feature_per_frame)
                {
                    ...
                    if (ESTIMATE_TD)
                    {
                        ProjectionTdFactor *f_td = new ProjectionTdFactor(pts_i, pts_j, it_per_id.Event_feature_per_frame[0].velocity, it_per_frame.velocity,
                                                                          ...
                                                                          it_per_id.Event_feature_per_frame[0].uv.y(), it_per_frame.uv.y());
                        ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(f_td, loss_function,
                                                                                        vector<double *>{para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[0], para_EventFeature[Event_feature_index], para_Td[0]},
                                                                                        vector<int>{0, 3});
                        marginalization_info->addResidualBlockInfo(residual_block_info);
                    }
                    else
                    {
                        ProjectionFactor *f = new ProjectionFactor(pts_i, pts_j);
                        ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(f, loss_function,
                                                                                       vector<double *>{para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[0], para_EventFeature[Event_feature_index]},
                                                                                       vector<int>{0, 3});
                        marginalization_info->addResidualBlockInfo(residual_block_info);
                    }
                }
...
void Estimator::optimizationStereo()
{   
    frame_cnt_ ++;
    ceres::Problem problem;
    ceres::LossFunction *loss_function;
    ...
    for (int i = 0; i < NUM_OF_CAM; i++)//仍然只优化左相机到imu的内参
    {
        ceres::LocalParameterization *local_parameterization = new PoseLocalParameterization();
        problem.AddParameterBlock(para_Ex_Pose[i], SIZE_POSE, local_parameterization);
        if (!ESTIMATE_EXTRINSIC)
    ...
    for (auto &it_per_id : f_manager.stereo_feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            if(it_per_frame.getDepth() > 0 ){
                Vector3d pts_j_right = it_per_frame.pointRight;
                if(imu_i != imu_j){
                    // if(!outlierCheck(imu_i, imu_j, pts_i, pts_j_right, para_Feature[feature_index][0])){ 
                    //     outlier_cnt++; 
                    //     continue; 
                    // }
                    ...
                    Eigen::Matrix2d sqrt_info = it_per_frame.getOmegaRight(); 
                    f->setInfoMatrix(sqrt_info); 
   ...
   // 对于事件线特征
    ...
    options.linear_solver_type = ceres::DENSE_SCHUR;
    // options.num_threads = 2;//启用多线程（毕竟三种特征一起，数量太多了）
    options.trust_region_strategy_type = ceres::DOGLEG;
    options.max_num_iterations = NUM_ITERATIONS;
    options.use_explicit_schur_complement = true;
    //options.minimizer_progress_to_stdout = true;//可能需要改为false
    //options.use_nonmonotonic_steps = true;
    if (marginalization_flag == MARGIN_OLD)
        options.max_solver_time_in_seconds = SOLVER_TIME * 4.0 / 5.0;
    else
    ...
    // double2vector();
    // double2vector2();   // Line pose change
    stereo_double2vector3();   // Line pose change
    // f_manager.removeLineOutlier(Ps,tic,ric);   // remove Line outlier
            ...
            for (auto &it_per_id : f_manager.stereo_feature)
            {
                ...
                for (auto &it_per_frame : it_per_id.feature_per_frame)
                {
                    ...
                    if(imu_i != imu_j){
                        ...
                        ProjectionTwoFrameOneCamFactor *f_td = new ProjectionTwoFrameOneCamFactor(pts_i, pts_j, it_per_id.feature_per_frame[0].velocity, it_per_frame.velocity,
                                                                            it_per_id.feature_per_frame[0].cur_td, it_per_frame.cur_td);
                        if(it_per_frame.getDepth() > 0){ // set new variance 
                            //  pts_j = it_per_frame.new_point; // stereo point
                            // Eigen::Matrix2d sqrt_info = it_per_frame.getOmega(); 
                            // f_td->setInfoMatrix(sqrt_info); 
                        }
                        ResidualBlockInfo *residual_block_info = new ResidualBlockInfo(f_td, loss_function,
                                                                                        vector<double *>{para_Pose[imu_i], para_Pose[imu_j], para_Ex_Pose[0], para_Feature[feature_index], para_Td[0]},
                                                                                        vector<int>{0, 3});
